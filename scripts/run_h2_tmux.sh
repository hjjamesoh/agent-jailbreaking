...
session_name="${1:-h2-agent-step}"
config_path="${2:-configs/h2_agent_step_llama31.yaml}"
gpu_id="${3:-${CUDA_VISIBLE_DEVICES:-}}"
log_path="runs/${session_name}_$(date +%Y%m%d_%H%M%S).log"

if [[ -z "$gpu_id" ]]; then
  echo "usage: bash scripts/run_h2_tmux.sh <session> <config> <physical_gpu_id>" >&2
  echo "example: bash scripts/run_h2_tmux.sh h2-agent-step configs/h2_agent_step_llama31.yaml 0" >&2
  exit 2
fi
...
tmux new-session -d -s "$session_name" \
  "env CUDA_VISIBLE_DEVICES='$gpu_id' bash scripts/run_h2_all.sh '$config_path' '$log_path'"
